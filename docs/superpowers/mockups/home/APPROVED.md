...
2. **Ticker** — full-width moving tape (~30s loop per sequence, pauses on hover **on fine pointers only** — see the accessibility divergence under "Ticker loop" below — `aria-hidden` on the tape, static under reduced motion): quiet monochrome pairs + uncostumed real stats. Items: SWIFT/SWIFTUI · PYTORCH/TENSORFLOW · CUDA/C++ 35.31× · PYTHON/NUMPY · APP STORE 5.0★ · MECHANICAL 7 YRS. **No stock-price styling, no green gains.**
...
- **Hover-pause is gated to `@media (hover: hover) and (pointer: fine)`.** Ungated it is the touch `:hover` trap — a tap applies `:hover`, the tape freezes, and it only resumes when the user taps elsewhere. Omar hit exactly that on the phone. A sticky tap was never a pause mechanism, so gating it loses nothing real.
