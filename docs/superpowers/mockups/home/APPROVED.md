...
**Light page bands: resolve the sky tint at a LENGTH, not at 100% (standing correction, added 2026-08-28 after the THIRD occurrence).**
Every mockup so far has drawn its light band as a full-height wash `#eef4fb → #fbfbfd`, and on all three the page's accent eyebrow (12px/900, which is NOT WCAG large text) landed on the tinted part of that wash at ~4.2–4.3:1: Projects 4.37:1, About 4.28:1, Contact 4.29:1. Each build then had to correct it independently, to the same value, for the same reason. **That is one systematic defect in how the mockups are drawn, not three bugs.** From now on a light band's tint stops at `100px` — a length, so it does not move when the band's height changes — which puts every word of header copy on plain `#fbfbfd` (4.54:1) and leaves the tint under the nav, where the nav's own colours have far more headroom. **Draw the mockup this way in the M-task**; do not draw a full-height wash and leave the B-task to pay for it a fourth time. Dark is unaffected and keeps its full-height fade.
