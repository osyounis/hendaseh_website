...
/* The octocat now comes from `@/components/BrandMarks` -- it had been inlined
   here, in the case-study template and on Contact, and the Home CTA row would
   have made a fourth copy of the same 400-character path. The original comment
   argued against pulling it from `react-icons` (so the icon set cannot change
   under the page when a dependency updates); a local module keeps exactly that
   guarantee with one copy instead of four.

   `.projects-mini svg:not(.link-glyph)` still owns the SIZE here -- 15px, the
   mini pill's own step -- because it is a compound selector and outranks
   `.brand-mark-github`'s 1.1em. The card renders identically to before.

   Apple's glyph stays inline below: it has exactly one consumer, and moving a
